...
class ExternalPoseBridgeNode(Node):
    def __init__(self) -> None:
        ...
        self.declare_parameter("legacy_input_pose_topic", "")
        self.declare_parameter("output_pose_topic", "")
        self.declare_parameter("restamp_with_local_clock", False)
        ...
        self.input_pose_topic = str(self.get_parameter("input_pose_topic").value).strip()
        self.legacy_input_pose_topic = (
            str(self.get_parameter("legacy_input_pose_topic").value).strip()
            or legacy_vio_input_topic(self.drone_id)
        )
        ...
        self.restamp_with_local_clock = bool(
            self.get_parameter("restamp_with_local_clock").value
        )
        ...
        self.last_input_topic = ""
        self.subscribed_topics: list[str] = []

        self._create_input_subscriptions()
        ...
        self.get_logger().info(
            "External pose bridge started: "
            f"{', '.join(self.subscribed_topics)} -> {self.output_pose_topic}"
        )

    def _create_input_subscriptions(self) -> None:
        if self.input_pose_topic:
            self._subscribe(self.input_pose_topic)
            return

        canonical_topic = external_pose_input_topic(self.drone_id)
        self._subscribe(canonical_topic)
        if self.legacy_input_pose_topic != canonical_topic:
            self._subscribe(self.legacy_input_pose_topic)
    ...
    def _subscribe(self, topic: str) -> None:
        ...
        self.create_subscription(
            PoseStamped,
            topic,
            lambda msg, source_topic=topic: self.pose_callback(msg, source_topic),
            10,
        )
    ...
    def pose_callback(self, msg: PoseStamped, source_topic: str) -> None:
        self.latest_pose = msg
        self.last_pose_s = self.now_s()
        self.last_input_topic = source_topic
    ...
    def publish_loop(self) -> None:
        ...
        if self.restamp_with_local_clock or _stamp_is_zero(pose_msg):
